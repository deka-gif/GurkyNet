...
import {
  MobileCompactProductTile,
  MobileCompactProductSkeleton,
} from '../../components/catalog/MobileCompactProductTile';
 ...
 * WEB MOBILE UX: clean ← Pulsa flow + compact product grid (desktop layout preserved).
 */
export const PulsaPage = () => {
  ...
  return (
    <div
      className={`dashboard-page container mx-auto max-w-5xl ${selectedProduct ? MOBILE_STICKY_ACTION_PAD : ''}`}
      id="pulsa-page-root"
    >
      {/* —— WEB MOBILE: APK-like purchase flow —— */}
      <div className="md:hidden space-y-4 max-w-lg mx-auto">
        <MobilePurchaseHeader title="Pulsa" backTo="/dashboard" />
        ...
        {!provider ? (
          <div className="py-8 text-center text-gray-400">
            <p className="text-xs font-medium">Masukkan nomor HP untuk melihat nominal.</p>
          </div>
        ) : productsLoading ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            {displayProducts.map((opt) => (
              <MobileCompactProductTile
                key={opt.id || opt.code}
                product={opt}
                selected={selectedProduct?.id === opt.id}
