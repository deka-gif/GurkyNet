...
export const RiwayatPage = () => {
  ...
  // Interactive selected transaction for modal invoice receipt
  ...
  const filteredTransactions = transactions.filter((tx) => {
    ...
    // 3. Category (Service Name)
    const matchCategory = selectedCategory === 'all' || tx.serviceName.toLowerCase().includes(selectedCategory.toLowerCase());
    ...
    // 5. Date Period & Custom Date Search
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 container mx-auto max-w-5xl" id="riwayat-page-root">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight font-sans">Riwayat Transaksi</h2>
          <p className="text-sm text-gray-500">Cari, filter, dan unduh struk transaksi PPOB dan mutasi saldo Anda.</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-3xl p-5 border border-gray-100 shadow-xl shadow-gray-200/20 space-y-4">
        ...
        <div className="pt-3 border-t border-gray-50 grid grid-cols-1 md:grid-cols-12 gap-4">
          ...
          <div className="md:col-span-4">
            <label className="block text-[10px] font-extrabold text-gray-400 uppercase tracking-wider mb-1.5 ml-1">Kategori Layanan</label>
            <select
          ...
          {/* Nominal Filter */}
          <div className="md:col-span-4">
            <label className="block text-[10px] font-extrabold text-gray-400 uppercase tracking-wider mb-1.5 ml-1">Rentang Nominal</label>
            <select
          ...
          {/* Custom Date search picker */}
          <div className="md:col-span-4">
            <div className="flex justify-between items-center mb-1.5 ml-1">
              <label className="block text-[10px] font-extrabold text-gray-400 uppercase tracking-wider">Cari Tanggal Spesifik</label>
              {customDate && (
                <button 
      ...
      {loading ? (
        ...
        <div className="p-16 text-center bg-white rounded-3xl border border-gray-100 shadow-xl shadow-gray-200/20 space-y-3">
          ...
          <p className="text-xs text-gray-400">Tidak ada data transaksi yang cocok dengan filter atau kata pencarian Anda.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-gray-400 font-extrabold uppercase tracking-wide">{tx.serviceName}</span>
                    <span className="text-gray-300">•</span>
                    <span className="text-[10px] text-gray-500 font-bold tracking-wider">{tx.transactionCode}</span>
                  </div>
                  <h4 className="font-black text-gray-900 text-sm leading-tight mt-1 group-hover:text-primary-600 transition-colors">
                    {tx.productName}
                  </h4>
                  <p className="text-[10px] text-gray-400 mt-0.5">Tujuan: {tx.targetNo} • {new Date(tx.date).toLocaleString('id-ID', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</p>
                </div>
              </div>
              ...
              <div className="flex justify-between md:justify-end items-center gap-6 border-t md:border-t-0 pt-3 md:pt-0 border-gray-50">
                ...
                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase flex items-center gap-1.5 ${
                  ...
                  {tx.status === 'sukses' && <CheckCircle2 className="w-3 h-3 text-emerald-600" />}
                  {tx.status === 'pending' && <Clock className="w-3 h-3 text-amber-600 animate-pulse" />}
                  {tx.status === 'gagal' && <XCircle className="w-3 h-3 text-red-600" />}
                  <span>{tx.status}</span>
                </span>
              </div>
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
            ...
            >
              ...
              <div className="text-center space-y-2">
                ...
                <h4 className="font-extrabold text-gray-900 text-sm tracking-wide uppercase">Struk Transaksi Resmi</h4>
                <p className="text-[10px] text-gray-400">Dicetak melalui portal pintar GurkyNet secara digital.</p>
              </div>

              {/* Decorative dotted separator lines */}
              <div className="border-t border-dashed border-gray-200/80 pt-4 space-y-4">
                ...
                <div className="text-center py-2">
                  ...
                  <h3 className="text-2xl font-black text-primary-600 mt-1">{formatIDR(selectedTx.amount)}</h3>
                </div>
                ...
                <div className="space-y-3.5 text-xs font-bold text-gray-500 px-1">
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900 flex items-center gap-1.5">
                      ...
                      >
                        {copiedId === selectedTx.transactionCode ? <Check className="w-3 h-3 text-emerald-500" /> : <Copy className="w-3 h-3" />}
                      </button>
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900">{new Date(selectedTx.date).toLocaleString('id-ID')}</span>
                  </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900 font-extrabold uppercase text-[10px] tracking-wide">{selectedTx.status}</span>
                  </div>

                  {/* Serial Number Note block */}
                  <div className="border-t border-gray-50 pt-3 flex flex-col gap-1">
                    ...
                    <p className="text-gray-900 text-[11px] leading-relaxed break-all bg-gray-50 p-3 rounded-2xl border border-gray-100 font-mono tracking-tight font-medium">
                      {selectedTx.note || 'Transaksi diproses dengan sukses oleh mitra provider.'}
                    </p>
                  </div>
                </div>
