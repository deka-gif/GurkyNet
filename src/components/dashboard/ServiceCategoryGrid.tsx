    ...
    <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-lg shadow-slate-200/40 md:p-7">
      <div className="mb-5 flex items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-400">Pilih kategori untuk mulai transaksi</p>
        </div>
      </div>
          ...
          return (
            <motion.button
              ...
              className={`group relative flex cursor-pointer flex-col items-center rounded-2xl border bg-white px-3 py-4 text-center transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/30 ${
                isActive
                  ? 'border-primary-300 shadow-md shadow-primary-600/10 ring-2 ring-primary-500/20'
                  : 'border-slate-100 hover:border-primary-200'
              }`}
            >
              {cat.badge ? (
                <span
                  className={`absolute right-2 top-2 rounded-md px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wide ${
                    cat.badge === 'Promo'
              ...
              <div
                className={`mb-3 flex h-14 w-14 items-center justify-center rounded-2xl border shadow-sm transition-transform duration-200 will-change-transform group-hover:scale-105 ${cat.tone}`}
              >
